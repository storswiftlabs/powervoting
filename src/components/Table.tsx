...
export default function Table({ title, list, subTitle, tdCss }: Props) {
  return (
      ...
      <tbody className="divide-y divide-[#313D4F]">
        {list.map((item: any) => (
          <tr key={item.name} className="divide-x divide-[#313D4F]">
            <td
              style={{ ...tdCss }}
              className="w-[280px] whitespace-nowrap py-9 px-8 text-xl text-[#8896AA]"
            >
              {item.name}
            </td>
            <td style={{ ...tdCss }} className="py-5 px-4 text-xl text-white">
              {item.comp}
            </td>
          </tr>
        ))}
